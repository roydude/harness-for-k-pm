...
Use this snippet to register the Figma MCP server in `~/.claude/settings.json` as a streamable HTTP server with bearer auth pulled from your env.
...
- Add the snippet above to `~/.claude/settings.json` under `[mcp_servers.figma]`, and enable `[features].rmcp_client = true` (or `experimental_use_rmcp_client = true` on older releases).
- Restart Claude Code (CLI/IDE) after updating config and env vars.
